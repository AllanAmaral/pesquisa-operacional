...
Lmais = model.addVars(jobs, vtype=GRB.CONTINUOUS, name="Lmais")
Lmenos = model.addVars(jobs, vtype=GRB.CONTINUOUS, name="Lmenos")
...
for j in jobs:
    if j != 0:
        model.addConstr(Lmax >= Lmais[j] - Lmenos[j])

for j in jobs:
    if j != 0:
        model.addConstr(Lmais[j] - Lmenos[j] == C[j] - d[j])
