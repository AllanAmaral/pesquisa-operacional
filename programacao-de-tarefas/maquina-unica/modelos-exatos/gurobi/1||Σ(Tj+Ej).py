...
E = model.addVars(jobs, vtype=GRB.CONTINUOUS, name="Ej")

# Função objetivo
model.setObjective(gp.quicksum(T[j] + E[j] for j in jobs if j!=0), GRB.MINIMIZE)
...
if model.Status == GRB.OPTIMAL:
    print("\nJobs, atrasos e adiantamentos:")
    for j in jobs:
        if j != 0:
            print(f"J{j}, T={T[j].X:.2f} e E={E[j].X:.2f}")
